...
The second assumption is that a unique BLP should exist. Our visualizations show that both of our main predictor and outcome variables have a heavy right-tail, indicating a potential non-finite variance. We see that vif values of our main interest variables below 5 (vif value for main predictor = `{r} round(vif4[[1]], 2)`), meaning there is no perfect collinearity. Although we see a non-infinite variance from our sample for exploration (mean AGI: `{r} round(var_pred, 2)` , mean charitable contribution: `{r} round(var_outc, 2)`), it should be noted that the underlying distribution may still have non-finite variance, which may violate our assumption for the presence of a unique BLP.

#### 8. Model Results and Interpretation

The models significantly predict mean charitable contributions (p \< .001), with adjusted R-squared values indicating varying explanatory power. Models 4, including wealth/investment income, show higher explanatory power than Models 1 and 2 (which only include AGI and AGI squared). Specifically, Model 1 (mean AGI) explains 60.2%, of the variance, while Model 2 (with AGI squared) increases this to 70.6%. Adding wealth/investment income (Model 4) further increases its explanatory power to 78.5%. This suggests income, wealth/investment, and salary significantly influence charitable giving.

On average, individuals with higher dividend earnings are expected to donate more, with every \$1,000 increase in mean dividend income associated with additional \$49 in charitable contributions. The model captures a U-shaped relationship between mean AGI and mean charitable donations, having a positive coefficient for the squared term for mean AGI. For instance, at very low AGI levels of \$15,000, individuals may have significant financial constraints. As their income slightly rises to \$25,000, they might still be focused on meeting basic needs, and charitable giving might remain relatively low or even decrease as they have less disposable income after taking on new expenses. As income continues to increase to \$50,000 or \$75,000, individuals might have more discretionary income, and their charitable giving could increase at an increasing, non-linear rate. Although we see positive correlations between charitable contributions with mean cap gain and mean rent royalty, we lack sufficient evidence for the relationship to exist in the larger population.

In a broader sense, these findings indicate that charitable giving is a complex behavior influenced by a combination of factors. Stable income, and income from investments, such as dividends, both play a role. The results corroborate our initial hypothesis of non-linear, U-shaped relationship between AGI and charitable contributions. These results may be useful for charitable organizations and researchers exploring complex dynamics associated with charitable behaviors.
...
stargazer(m1, m3, m5, m6, m7, m8, m9,
  type = ifelse(knitr::is_latex_output(),"latex", "html"),
  # type = "text",
  header = F,
  se = list(se1, se3, se5, se6, se7, se8, se9),
  column.sep.width = ".001pt",
  column.labels = c("(1)", "(3)", "(5)", "(6)", "(7)", "(8)", "(9)"),
  model.numbers = F,
  font.size = "tiny",
